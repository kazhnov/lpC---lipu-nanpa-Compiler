...
NodeExpression parseExpr(Tokens *tokens) {
      if (tokenPeek(tokens).type == TOKEN_NUMBER) {
            ...
            return (NodeExpression) {.lon = true, .type = NanpaExpr, .value = token};
      }
      if (tokenPeek(tokens).type == TOKEN_NAME) {
            Token token = tokenConsume(tokens);
            return (NodeExpression) {.lon = true, .type = NimiExpr, .value = token};
      }
...
NodeO parseO(Tokens *tokens) {
      if (tokenPeek(tokens).type == TOKEN_O) {
            ...
            NodeExpression expr = parseExpr(tokens);
            if (!expr.lon) {
            ...
            return (NodeO){.lon = true, .type = type, .name = name, .expr = expr};
      }
...
void generateExpression(NodeExpression expr) {
      if (expr.type == NanpaExpr) {
            push(atoi(expr.value.value));
      } else if (expr.type == NimiExpr) {
            if (!hasNameMap(&vars, expr.value.value)) {
                  printf("Undefined identifier %s\n", (char*) expr.value.value);
                  exit(1);
            }
            size_t offset = getNameMap(&vars, expr.value.value);
            char str[20];
            printf("    mov rax, rsp\n"
                   "    add rax, %ld\n", (-offset + stackOffset) * 8);
